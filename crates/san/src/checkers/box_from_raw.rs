        ...
        for (bb, block_data) in body.basic_blocks.iter_enumerated() {
            ...
            if let Some(state) = flow.state_before_terminator(tcx, body, bb) {
                if let Some(arg_local) = first_arg_local(args) {
                    // For FROM_RAW calls on already-freed pointers, ownership_double_free
                    // in the flow checker fires the precise finding — suppress the Tier-1
                    // warning here to avoid redundant reporting.
